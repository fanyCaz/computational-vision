...
def circulos():    
    ima1 = Image.open("mina_chica.jpeg")
    imagen = ima1.load()
    ...
    for i in range(ancho):
        for j in range(alto):
            ...
            if fabs(grad) > 0:
                ...
                ycm = (ancho/2) - yc
                #centro = (xc)
                if ycm >= 0 and int(xcm) < alto-1 and int(xcm) >= 0 and ycm < ancho-1:
                    print(f"xcm {xcm} alto {alto} ycm {ycm} ancho {ancho}")
                    #print(frec[int(xcm)][int(ycm)])
                    if int(xcm) + 1 < alto:
                        frec[int(ycm)][int(xcm)] += 1
 #   agregar los votados
                          ...
                          for dx in range(-rango, rango):
                               for dy in range(-rango, rango):
                                    if not (dx == 0 and dy == 0):
                                        if y + dy < ancho and y + dy >= 0 and x + dx >=0 and x + dx < alto:
                                             w = frec[y+dy][x+dx]
                                             if w >0:
                                                 if v -rango >=w:
                                                     frec[y][x] = v + w
