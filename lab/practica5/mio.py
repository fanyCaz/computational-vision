...
def wrap(posibles, image): #metodo de wrappping o marcha de jarvis
    env_hull = min(posibles)
    wrapp = list()
    i = 0
 
    while False is not True:
        wrapp.append(env_hull)
        final = [posibles[0], 0.0]
            
        for j in range(0, len(posibles)): #para dibujar las lineas segun su angulo y los puntos seleccionados 
            if len(wrapp) > 1:
                inclina = toma_giro(wrapp[i-1], wrapp[i], posibles[j])
            else:
                toma = (wrapp[0][0], wrapp[0][1]-1) 
                inclina = toma_giro(toma, wrapp[i], posibles[j])
...
#aplica wrapp 
    ...
    for i in range(1, len(wrapp)):
        draw.rectangle((wrapp[i-1][0], wrapp[i-1][1], wrapp[i][0], wrapp[i][1]) , outline="blue")
        draw.line((wrapp[i-1][0], wrapp[i-1][1], wrapp[i][0], wrapp[i][1]) , fill=(255,0,0))
    #draw.rectangle((wrapp[i-1][0], wrapp[i-1][1], wrapp[i][0], wrapp[i][1]) , outline="blue")
    #draw.rectangle((wrapp[len(wrapp)-1][0], wrapp[len(wrapp)-1][1], wrapp[0][0], wrapp[0][1]) , outline="blue")
    
    draw.line((wrapp[len(wrapp)-1][0], wrapp[len(wrapp)-1][1], wrapp[0][0], wrapp[0][1]) , fill=(255,0,0))
    draw.rectangle((wrapp[len(wrapp)-1][0], wrapp[len(wrapp)-1][1], wrapp[0][0], wrapp[0][1]) , outline="blue")
